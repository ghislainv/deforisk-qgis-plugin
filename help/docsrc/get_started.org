...
Considering a total deforestation $D$ for the next $Y$ years, the adjustment factor is $\rho = D / (A \sum_i n_{i} \theta_{m,i})$, the absolute rate is $\theta_{a,i} = \rho \theta_{m,i}$, and the deforestation density is $\delta_{i} = \theta_{a,i} \times A / Y$. The deforestation density is used to predict the amount of deforestation (in ha/pixel/yr) for each forest pixel belonging to a given class of deforestation risk for the next $Y$ years (for notations, see details [[file:plugin_api.html#defrate-table][here]]).
